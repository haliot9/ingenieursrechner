...
export function getOttoDiagramSpec(values: Record<string, VariableState>): DiagramSpec | null {
  const statePoints: DiagramStatePoint[] = [1, 2, 3, 4].map(state => ({
    id: String(state), label: `Z${String.fromCharCode(8320 + state)}`,
    p: get(values, `p${state}`), v: get(values, `v${state}`), T: get(values, `T${state}`), s: get(values, `s${state}`),
  }))
  ...
  return {
    ...
    segments: [
      ...
      { from: '2', to: '3', processType: 'isochoric', label: '2→3 isochor' },
      { from: '3', to: '4', processType: 'adiabatic', label: '3→4 isentrop' },
      { from: '4', to: '1', processType: 'isochoric', label: '4→1 isochor' },
    ],
    energyFlows: [
