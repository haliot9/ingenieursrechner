...
export function CalculationStoryDisplay({ story, focused = false, onToggleFocus, focusControlRef }: { story: DisplayStory; focused?: boolean; onToggleFocus?: () => void; focusControlRef?: Ref<HTMLButtonElement> }) {
  ...
  return <section className="calculation-story" aria-labelledby="calculation-story-title">
    <header className="calculation-story-heading"><div><p className="eyebrow">Herleitung</p><h2 id="calculation-story-title">{story.story.title}</h2><p>Die Rechenwerte stammen unverändert aus dem Solver. Der sichtbare Weg zeigt Modell, Bedingungen und Rechenschritte getrennt.</p></div><div className="calculation-story-controls"><button type="button" aria-pressed={mainPathOnly} onClick={() => setMainPathOnly(value => !value)}>Nur Hauptpfad</button>{onToggleFocus && <button type="button" ref={focusControlRef} data-calculation-story-focus-control aria-label={focused ? 'Rechenweg-Fokus verlassen' : 'Rechenweg fokussieren'} aria-pressed={focused} onClick={onToggleFocus}>{focused ? 'Fokus verlassen' : 'Rechenweg fokussieren'}</button>}</div></header>
